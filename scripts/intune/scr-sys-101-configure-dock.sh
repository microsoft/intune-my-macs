...
# Detect macOS "Tahoe" (26) Apps.app replacement for Launchpad
launcherApp=""
if [[ -e "/System/Applications/Apps.app" ]]; then
  # macOS 26+ (Tahoe) new unified Apps launcher
  launcherApp="/System/Applications/Apps.app"
elif [[ -e "/System/Applications/Launchpad.app" ]]; then
  launcherApp="/System/Applications/Launchpad.app"
fi
...
# Remove empty entries (if neither Apps.app nor Launchpad.app existed)
cleanedDockApps=()
for a in "${dockapps[@]}"; do
  [[ -z "$a" ]] && continue
  cleanedDockApps+=("$a")
done
dockapps=(${cleanedDockApps[@]})
